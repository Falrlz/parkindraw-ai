...
def generate_eda_report():
    ...
    if not os.path.exists(images_path):
        print(
            "Error: run `uv run --extra data parkindraw-audit "
            "--config configs/data/audit.yaml` first."
        )
        return
    ...
    print(
        f"   - Healthy Subjects (Label 0): {report['healthy_subjects']} ({report['healthy_subjects'] / len(df_sub):.1%})"
    )
    print(
        f"   - Parkinson Subjects (Label 1): {report['parkinson_subjects']} ({report['parkinson_subjects'] / len(df_sub):.1%})"
    )
    ...
    for dtype, count in drawing_counts.items():
        print(
            f"   - {dtype.capitalize():<10}: {count} images ({count / len(df_img):.1%})"
        )

    print("\n3. IMAGE DIMENSION & RESOLUTION DISTRIBUTION:")
    res_df = (
        df_img.groupby(["width", "height", "channels"]).size().reset_index(name="count")
    )
    for _, row in res_df.iterrows():
        print(
            f"   - Resolution: {row['width']}x{row['height']} ({row['channels']} channels) -> {row['count']} images"
        )
    ...
    if not df_dup.empty:
        ...
        for dup_id, group in df_dup.groupby("duplicate_group_id"):
            ...
            print(
                f"     * Group {dup_id}: {len(group)} images across subjects {subjects} ({drawings}, {classes})"
            )

    print("\n5. INTEGRITY & QUALITY CHECKS:")
    print(f"   - Corrupted Images Count    : {report['corrupted_images_count']}")
    print(
        "   - Drawings per Subject      : Exactly 9 drawings per subject (1 Circle, 4 Meander, 4 Spiral)"
    )
